...
void CsvUtil::addFileData(const std::string &rSCsvFilePath)
{
    ...
    auto linesVec = tFileLoadUtil.getDataLines(rSCsvFilePath);	//取出一个文件的数据,返回vector<std::string>
    StrVec strsVec;
    StrDict dict;
    ...
    {
        strsVec = tStringUtil.split(linesVecIter.c_str(), ",");//取出该文件第i行的数据.以,分割,得到的vector<std::string>
        dict.push_back(strsVec);//存入dict
    }
