...
typedef std::vector<std::string> StrVec; //字符串数组(向量)

typedef std::vector< StrVec > StrDict;   //二维字符串数组(向量)

typedef std::map< std::string, StrDict> CsvMap;  //字典;
...
public:
     ...
     *
     *  @param iRow          is row num,表示第几行,编号是从0开始的;
